...
const MainPage = props => {
  ...
  const theOddsApiSportsUrl = `https://api.the-odds-api.com/v3/sports/?apiKey=${THE_ODDS_API_KEY}`
  ...
  useEffect(() => {
    ...
    const getTheOddsSports = async () => {
      setIsLoading(true);
      const a = await fetch(theOddsApiSportsUrl);
        ...
        <span>Loading...</span>
          ...
          <table>
            ...
            <tbody>
              {data.map(e => {
                return (
                  <tr key={e.key} onClick={() => history.push(`/sportDetails/${e.key}`)}>
                    <td>
                      {e.group}
                    </td>
                    <td>
